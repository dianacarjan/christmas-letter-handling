...
class LetterControllerIntegrationTest extends LocalStackTestContainer {
    ...
    void givenLetter_whenPublishingToSNS_thenMessageQueueIsReceived(CapturedOutput output) throws Exception {
        ...
        await().atMost(1, TimeUnit.SECONDS).until(() -> output.getAll().contains(expectedPublisherLog));
    }

    @ParameterizedTest
    @MethodSource("com.christmas.letter.sender.helper.LetterUtils#provideInvalidLetters")
    void givenInvalidLetter_whenSendingLetter_thenTopicIsEmpty(Letter letter, String expectedOutput) throws Exception {
        // Arrange & Act & Assert
        mockMvc.perform(post(LETTER_CREATE_PATH)
                        .contentType(MediaType.APPLICATION_JSON)
