...
public class LetterSqsListener {
    ...
    @SqsListener(value = "${letter-processor.aws.sqs.queue-url}", acknowledgementMode = SqsListenerAcknowledgementMode.MANUAL)
    public Letter listen(@Valid @Payload LetterMessage letterMessage, Acknowledgement acknowledgement) {
        log.info("Letter Message received {}, on the listen method", letterMessage);
        Letter savedLetter = letterRepository.save(LetterMapper.INSTANCE.letterMessageToLetter(letterMessage));
